...
const handleActionString = '  const handleAction = async () => {';
...
content = content.replace(handleActionString, handleGoogleLoginStr + handleActionString);

const buttonTarget = `        <div className="mt-8 pt-6 border-t border-white/5 text-center flex flex-col gap-3">`;
const newButtonStr = `        <div className="relative mt-6">
...
content = content.replace(buttonTarget, newButtonStr);

fs.writeFileSync(path, content);
console.log("Google replacements done!");
